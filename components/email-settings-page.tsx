      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(320px,0.72fr)]">
        <section className="rounded-lg border bg-card p-5 shadow-operational" aria-labelledby="delivery-preferences-title">
          ...
          <div className="mt-5 space-y-4">
            <div>
              <label htmlFor="recipient-email" className="text-sm font-medium">Personal recipient email</label>
              <Input id="recipient-email" type="email" autoComplete="email" className="mt-2" value={form.recipient_email} onChange={(event) => setForm((current) => ({ ...current, recipient_email: event.target.value }))} />
              <p className="mt-1.5 text-xs text-muted-foreground">Used by Daily Brief, Weekly Summary, Test, and as fallback for Manager Summary.</p>
            </div>
            <div>
              <label htmlFor="manager-recipient-email" className="text-sm font-medium">Manager recipient email</label>
              <Input id="manager-recipient-email" type="email" autoComplete="email" className="mt-2" value={form.manager_recipient_email ?? ""} placeholder="e.g. delivery.manager@bluestonex.com" onChange={(event) => setForm((current) => ({ ...current, manager_recipient_email: event.target.value || null }))} />
              <p className="mt-1.5 text-xs text-muted-foreground">Delivery Manager recipient for Manager Exception Reports. Falls back to personal email if blank.</p>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-lg border bg-card p-5 shadow-operational" aria-labelledby="manual-delivery-title">
          ...
          <div className="mt-5 grid gap-3">
            ...
            <Button variant="outline" className="min-h-11 justify-start" disabled={Boolean(busy)} onClick={() => send("/api/email/manager-summary", "Manager Summary")}><Send className="h-4 w-4" aria-hidden="true" />{busy === "/api/email/manager-summary" ? "Sending…" : "Send Manager Summary Now"}</Button>
          </div>
      ...
      <section className="mt-5 overflow-hidden rounded-lg border bg-card shadow-operational" aria-labelledby="email-activity-title">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[820px] text-sm"><thead className="bg-muted/70 text-left text-xs uppercase text-muted-foreground"><tr><th className="px-4 py-3">Email Type</th><th className="px-4 py-3">Recipient</th><th className="px-4 py-3">Sent</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Duration</th><th className="px-4 py-3">Trigger</th><th className="px-4 py-3">Failure Reason</th></tr></thead>
            <tbody>{activity.slice(0, 50).map((item) => <tr key={item.id} className="border-t"><td className="px-4 py-3 font-medium">{item.email_type}</td><td className="px-4 py-3">{item.recipient}</td><td className="px-4 py-3 tabular-nums">{formatTimestamp(item.sent_at)}</td><td className="px-4 py-3"><span className={`inline-flex items-center gap-1.5 font-medium ${item.success ? "text-emerald-700 dark:text-emerald-300" : "text-destructive"}`}>{item.success ? <CheckCircle2 className="h-4 w-4" aria-hidden="true" /> : <XCircle className="h-4 w-4" aria-hidden="true" />}{item.success ? "Sent" : "Failed"}</span></td><td className="px-4 py-3 tabular-nums">{item.duration_ms} ms</td><td className="px-4 py-3"><span className="inline-flex items-center gap-1.5"><Clock3 className="h-4 w-4 text-muted-foreground" aria-hidden="true" />{item.trigger_type}</span></td><td className="max-w-[200px] px-4 py-3 text-muted-foreground"><span className="block truncate" title={item.failure_reason ?? undefined}>{item.failure_reason ?? "—"}</span></td></tr>)}</tbody>
          </table>
        </div>
